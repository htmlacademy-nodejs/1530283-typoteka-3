...
describe(`API returns article based on unique search query`, () => {
  ...
  test(`Article has correct id`, () =>
    expect(response.body[0].id).toBe(`bOYRF-`));
});
...
describe(`API returns articles based on non-unique search query`, () => {
  ...
  test(`3 articles found`, () => expect(response.body.length).toBe(3));

  test(`1st has correct id`, () => expect(response.body[0].id).toBe(`P2ytE4`));

  test(`2nd has correct id`, () => expect(response.body[1].id).toBe(`bOYRF-`));

  test(`3rd has correct id`, () => expect(response.body[2].id).toBe(`9F-Pjp`));
});
