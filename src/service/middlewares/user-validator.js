...
const UserErrorMessage = {
  ...
  LAST_NAME_INVALID: `Фамилия не должно содержать цифр и специальных символов`,
  EMAIL_EMPTY: `Введите электронную почту`,
...
const schema = Joi.object({
  firstName: Joi.string().required().regex(UserRestriction.NAME_REGEX),
  lastName: Joi.string().required().regex(UserRestriction.NAME_REGEX),
  email: Joi.string().required().email(),
  password: Joi.string()
    .required()
    .min(UserRestriction.PASSWORD_MIN)
    .max(UserRestriction.PASSWORD_MAX),
  passwordRepeated: Joi.any().valid(Joi.ref(`password`)).required(),
  avatar: Joi.string()
    .allow(``)
    .custom((value, helper) => {
      if (!value) {
